...
- `HERMES_HOST_UID` and `HERMES_HOST_GID` MUST record the host user's `id -u` / `id -g` and are used (a) by the `hermes-init` compose service to chown bind-mounted data with the host GID as group, and (b) by the main `hermes` service's `group_add` so its writes remain host-readable and host-writable.
...
- `hermes-agent/compose.yaml` MUST define a `hermes-init` service that runs as root inside Docker, pre-creates the Hermes runtime subdirectories under `/opt/data/`, and chowns the bind mount to `HERMES_UID:HERMES_HOST_GID` with mode `2775` on directories and `0664` on files. The main `hermes` service MUST `depends_on: hermes-init` with `condition: service_completed_successfully`, MUST `group_add` `HERMES_HOST_GID`, and MUST set `umask 002` before exec'ing the Hermes binary. The combination ensures container writes are host-readable and host-writable without any host `sudo`.
