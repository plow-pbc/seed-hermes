...
6. Secret hygiene MUST pass: `hermes-agent/.env`, `hermes-agent/data/.env`, and `hermes-agent/data/auth.json` are git-ignored; tracked files MUST NOT contain GitHub token env vars, GitHub PAT prefixes, model keys, Plow `secret_key` values, or OAuth credentials; runtime logs MUST NOT be copied into commits or reports.
