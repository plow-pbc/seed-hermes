...
- `hermes-agent/Dockerfile` MUST install `jq` (via `apt-get install`) while running as `root`. Downstream hostex-history-ingest scripts depend on `jq`, and installing it at runtime via `docker exec` is lost on every container recreate.
- `hermes-agent/Dockerfile` MUST install `hermes-agent/entrypoint/seed-entrypoint.sh` to `/usr/local/bin/seed-entrypoint.sh` and set it as the image `ENTRYPOINT`. The script MUST run as root, MUST `chown -R HERMES_UID:HERMES_HOST_GID /opt/data` with mode `2775` on directories and `0664` on files, MUST run every executable under `/opt/data/bin/entrypoint.d/*.sh` in lexicographic order, and MUST `gosu`-drop to the image-baked hermes user before exec'ing the command. The upstream image's `ENTRYPOINT` MUST NOT be used directly — its own `chown` step would overwrite the host-gid grouping needed for shared writes.
- `hermes-agent/entrypoint/seed-entrypoint.sh` MUST defensively prepend `/opt/hermes/.venv/bin/hermes` when the first argv element starts with `-`. The upstream entrypoint dispatcher uses `command -v "$1"` to decide between `exec "$@"` and `exec hermes "$@"`, but `command -v "-p"` returns 0 because `-p` is a flag to `command` itself — and the subsequent `exec -p …` fails because bash's `exec` builtin does not accept `-p`. Defending in our entrypoint protects every service that uses our image, including per-profile services defined by downstream seeds.
- `/opt/data/bin/entrypoint.d/` is the canonical place for downstream seeds to drop boot-time hooks. Hooks MUST be idempotent, MUST be executable, and MUST exit non-zero only on real errors. Use this hook directory for any runtime patch that would otherwise be lost on container recreate (SDK monkey-patches, `apt install` of missing packages, symlink re-creation, etc.).
...
- `hermes-agent/.env` is local runtime state generated by `hermes-agent/scripts/prepare.sh`. It MUST set `COMPOSE_PROJECT_NAME`, `HERMES_CONTAINER_NAME`, `HERMES_UID`, `HERMES_GID`, `HERMES_HOST_UID`, and `HERMES_HOST_GID`, and MUST NOT be committed.
- `HERMES_UID` and `HERMES_GID` in `.env` MUST be `10000` — the canonical Hermes user UID/GID baked into the upstream `nousresearch/hermes-agent` image. They MUST NOT be set to the host user's `id -u` / `id -g`; downstream sidecars and installers (gbrain installer, airbnb-manager coordinator) read these values to target the correct user inside the container, and writing host IDs caused the v2 substrate run's `hermes user HOME is '/', expected '/opt/data'` failures.
- `HERMES_HOST_UID` and `HERMES_HOST_GID` MUST record the host user's `id -u` / `id -g`. `HERMES_HOST_GID` is used (a) by `seed-entrypoint.sh` as the directory group on `/opt/data`, and (b) by the main `hermes` service's `group_add` so the dropped hermes process can write files group-owned by the host gid.
- `hermes-agent/scripts/prepare.sh` MUST be idempotent and MUST migrate stale `.env` files where `HERMES_UID` / `HERMES_GID` are set to host IDs (the pre-fix default) — running it on a checkout produced by an earlier version of this seed MUST rewrite those keys to `10000`.
- `hermes-agent/compose.yaml` MUST set `user: "0:0"` on the `hermes` service so that `seed-entrypoint.sh` starts as root and can chown the bind mount and run privileged boot hooks. The service MUST `group_add: [HERMES_HOST_GID]` so the dropped hermes process retains the host gid as a supplementary group. The previous design used a separate `hermes-init` Compose service for the chown step; that service MUST NOT exist, because its responsibilities are folded into `seed-entrypoint.sh` (which runs from the same root context but inside the main service, where the upstream entrypoint expects to operate).
